...
- 519 исполняемых артефактов в полном замыкании `exports`;
- справочники и переиспользуемые условия;
- пять доменных операторов с закрытыми схемами и эталонными тестами;
- 158 исполняемых примеров с покрытием всех 237 кодов ошибок;
...
блока. Свободные строки `beneficiary.fl.birthPlace` и
`beneficiary.tax.foreignTaxResidency.address` не используются для определения
страны. Обязательность этих строк в применимых полных сценариях сохраняется.
